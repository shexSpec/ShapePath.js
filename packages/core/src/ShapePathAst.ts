...
export class EvalContext {
  ...
  tripleExprs: Map<string, SchemaNode> | null = null
  /**
...
export class Union extends Junction {
  t = "Union"
  evalPathExpr(nodes: NodeSet, ctx: EvalContext): NodeSet {
    return dedupe(this.exprs.reduce(
      (ret, expr) => ret.concat(expr.evalPathExpr(nodes, ctx))
      , [] as NodeSet
    ))
  }
 ...
 */
function itemKey (node: SchemaNode): string {
  return JSON.stringify(node)
}

/** the nodes, first occurrence of each kept */
function dedupe (nodes: NodeSet): NodeSet {
...
export class AxisStep extends Step {
  ...
  evalStep(nodes: NodeSet, ctx: EvalContext): NodeSet {
     ...
     */
    function walkTripleExpr(node: SchemaNode, seen: Set<any> = new Set()): NodeSet {
      if (node instanceof Array)
      ...
      switch ((<any>node).type) {
        case "EachOf":
        case "OneOf":
          return [node].concat(walkTripleExpr((<any>node).expressions, seen))
